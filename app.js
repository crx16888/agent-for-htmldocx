...
const DONG_KEHAN_STYLE_RULES =
  "董科含式内容约束：从一个具体场景进入；不是堆金句，而是通过一个选择抵达价值判断；保留事实边界；常用“不是A，而是B”“如果...”形成转折；结尾短、克制、留白。";
...
function getPreferences() {
  ...
  return {
    ...
    useDongKehan: els.useDongKehan.checked,
...
function setPreferences(preferences) {
  ...
  els.useDongKehan.checked = next.useDongKehan;
...
function buildOpeningScene(text, preferences) {
  ...
  if (depth <= 1) {
    return `# ${title}\n\n真正值得写下来的，不只是${anchor}本身，而是它背后那个反复出现的问题。`;
  }

  return `# ${title}\n\n有时候，一个问题不是在会议室里出现的，而是在你停下来、重新看一遍手上的材料时出现的。\n\n${anchor}看起来只是一个具体事项，但它真正提醒我们的，是方向、时间和选择之间的关系。`;
}
...
function improveParagraph(line, preferences) {
  ...
  if (preferences.useDongKehan && depth >= 3 && !/[。！？!?]不是/.test(improved) && /是|需要|应该|重要|关键/.test(improved)) {
    improved += `\n\n不是把它做得更热闹，而是先弄清楚它为什么值得被做。`;
  }

  if (preferences.useDongKehan && depth >= 4 && !isQuestion) {
...
function buildPromptPack() {
  ...
  return `请使用 dong-kehan skill 改善这篇公众号文章，并输出可直接进入公众号排版的 Markdown。

约束：
- ${DONG_KEHAN_STYLE_RULES}
...
function applyModeUi(shouldSwapContent = false) {
  ...
  els.modeHint.textContent = isWord
    ...
    : "公众号模式会先按内容工作流改善表达，再自动判断字号、行距和段落节奏。复制任务包可交给 Codex/模型做更强改稿。";
